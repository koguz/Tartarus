...
// D2 version: Run one individual on one board configuration AND track gene usage
// Updates gene.fitness, gene.used, gene.m_fitness using atomicAdd/atomicCAS
__global__ void run_all_boards_D2(
    gene* pop,
    ...
    // Use a compact bitfield to track used genes (G can be up to 128*383+1 = 49025)
    // But we only need to track unique (state, combo) pairs that were visited
    // For simplicity, we'll use a local array to track visited gene indices
    // Limited by register/local memory, so we track up to 80 unique genes (one per move max)
    int used_genes[80];
    int num_used = 0;
    ...
    // We only need fitness (sum) and used (count) for ratio calculation
...
int main(int argc, char** argv) {
    ...
    printf("=== D2 All-Boards Training ===\n");
    printf("N=%d, S=%d, K=%d generations\n", N, S, K);
    printf("Testing on ALL %d board configurations per individual\n", TOTAL_CONFIGS);
    printf("Using D2 crossover (gene m_fitness comparison)\n");
    srand(time(0));
    ...
    // File setup - D2 version uses different naming
    char fname[64], bname[64];
    sprintf(fname, "txt/r-D2-%d-%d-%d-%d.txt", N, S, K, L);
    sprintf(bname, "txt/b-D2-%d-%d-%d-%d.txt", N, S, K, L);
    ...
    for (int gen = 0; gen < K; gen++) {
        ...
        run_all_boards_D2<<<run_blocks, block_size>>>(pop, d_boards, F, G, S, N);
        CUDA_CHECK_KERNEL();
        CUDA_CHECK(cudaDeviceSynchronize());
        ...
        if (gen % 10 == 0 || gen == K - 1) {
            printf("Gen %d: Best=%.4f Avg=%.4f (D2, all %d boards)\n", gen, gen_best, gen_avg, TOTAL_CONFIGS);
